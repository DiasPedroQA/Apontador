# -*- coding: utf-8 -*-
"""
Módulo responsável por controlar a lógica de validação de caminhos de arquivos ou diretórios
com base no sistema operacional identificado.

Este módulo define a classe `CaminhoController`, que roteia o caminho informado
para o validador adequado (Windows, Linux/POSIX ou macOS), retornando o resultado
padronizado com base nas regras específicas de cada sistema.
"""
...
from models.validadores import validador_mac, validador_posix, validador_windows
...
class CaminhoController:
    ...
    def validar(self, caminho_entrada: str) -> ResultadoValidacao:
        """
        Valida o caminho de entrada com base no sistema operacional identificado.

        A função utiliza identificadores e validadores específicos para os sistemas
        Windows, Linux (POSIX) e macOS, retornando um dicionário com o resultado.

        Args:
            caminho_entrada (str): Caminho do sistema de arquivos a ser analisado.

        Returns:
            ResultadoValidacao: Dicionário contendo o caminho analisado,
            o sistema operacional identificado, se o caminho é válido,
